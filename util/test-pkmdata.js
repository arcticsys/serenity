...
if (!filename) {
    ...
    console.log('Usage: bun run test-pkmdata.js <filename>');
    process.exit(1);
}
    ...
    const req = http.request(options, (res) => {
        ...
        res.on('end', () => {
            ...
            try {
                ...
                fs.writeFile('./pkmdata-response', JSON.stringify(jsonResponse, null, 4), (err) => {
                    if (err) {
                ...
                fs.writeFile('./pkmdata-response', responseData, (err) => {
                    if (err) {
                        console.error('Error writing response to file:', err);
                    }
                });
